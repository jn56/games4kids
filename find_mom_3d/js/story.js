'use strict';
Meadow.Progress = {
  valleyFresh(){return {metBeaver:false,bridge:0,hammerFails:0,raft:0,raftFails:0,completed:false};},
  hillFresh(){return {metSquirrel:false,lights:[],focusHelp:[0,0,0],signal:false,reunited:false,completed:false};},
  forestFresh() {
  ...
  read() {
    try{
      ...
      const seen=new Set();
      state.arrangement=Array.from({length:4},(_,i)=>{
        const id=Array.isArray(raw.arrangement)?raw.arrangement[i]:null;
        if((id==='ribbon'?state.ribbon:state.flowers.includes(id))&&!seen.has(id)){seen.add(id);return id;}
        return null;
      });
...
Meadow.Story = class {
  ...
  interact(id) {
    ...
    if(id==='hedgehog'){
      ...
      if(!s.metHedgehog){
        this.say([L('栗栗','小米！你媽媽託我送一封信。她很安全，可是信卡在風管裡了……'),L('栗栗','我一著急，就把管子轉得亂七八糟。現在連光花也不開了。'),L('小米','我也有一點著急。我們先看一格，再接下一格，好不好？'),L('栗栗','好！點一下就能轉管子。讓風走過九格，從右下方吹出來，就能送信了。')],()=>{
      ...
      this.say([L('栗栗','媽媽的信和光花的線索，都收在你的手帳裡。想看就翻開吧！'),L('栗栗',s.flowers.length===3?'回到引路燈，把三朵花和紅髮帶排好。三條線索都符合，燈才會亮。':'每找到一朵光花，就會多一條點燈線索。到時候把線索合起來想。')]);return;
    }
  ...
  objective() {
    ...
    if(!s.windSolved)return {step:4,title:'修好九格送信風管',detail:'找栗栗打開風管。旋轉管口，讓風從入口走到出口。',emotion:'希望 · 一格一格想辦法',target:'hedgehog'};
    ...
    if(!s.lit)return {step:6,title:'解開引路燈的排列',detail:'回到引路燈，合併三條線索，排好花朵和紅髮帶。',emotion:'希望 · 我可以自己想出來',target:'lamp'};
