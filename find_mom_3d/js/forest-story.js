...
Meadow.ForestStory = class {
  ...
  readJournal(){
    ...
    this.say([['咕咕',`你已經找回 ${f.round} / 3 段旋律。前兩段照原來的順序，第三段從最後一個音倒著敲。`],['咕咕',f.separated?'媽媽在對岸休息亭，很安全。我們會一起走河谷步道。':'需要時可以重播，或請我留下圖案樂譜。旋律裡藏著媽媽的心意。']]);
  }
  interact(id){
    const g=this.game,f=g.state.forest;
    if(g.state.mode!=='playing')return;
    if(id==='owl'){
      if(!f.metOwl){this.say([['咕咕','藤蔓把去橋邊的小路擋住了。它們認得你媽媽的歌，我們用風鈴把歌找回來。'],['小米','我可以試試看！我記得她總會唱最後一段給我聽。'],['咕咕','合奏台有三段旋律，會越來越長。先看我示範，再敲一遍。最後一段是回聲，要倒著敲。'],['咕咕','聲音關著也沒關係，每個音都有自己的圖案。卡住了可以重播，或請我留下一點提示。']],()=>{f.metOwl=true;g.checkpoint(-1.4,5.5);g.toast('到風鈴合奏台試奏。每通過一段，藤蔓就會鬆開一點。',6000);});return;}
      if(f.separated&&!f.routeKnown){this.say([['小米','我才剛找到媽媽……現在又和她分開了。'],['咕咕','難過沒有關係。你看，她在對岸的亭子裡，很安全，也正向你揮手。'],['小米','我記得媽媽說，去山丘的路不只一條。你可以陪我找另一條嗎？'],['咕咕','當然！右邊的月光河谷步道可以繞過這裡。我們先去找修橋的河狸木木。']],()=>{f.routeKnown=true;g.checkpoint(3.8,-3.7);g.toast('找到新的路：跟著路牌，前往月光河谷。',5000);});return;}
      if(f.routeKnown){this.say([['咕咕','媽媽在對岸很安全。我會陪你走右邊的河谷步道，找木木幫忙。']]);return;}
  ...
  songComplete(){
    ...
    this.say([['咕咕','三段旋律都找回來了，連倒過來的回聲也難不倒你！'],['小米','藤蔓真的讓開了……那個身影，是媽媽！'],['媽媽','小米，我在這裡！慢慢走，媽媽等你。']],()=>g.toast('走過打開的藤蔓門，到媽媽身邊按互動。',6000));
  }
  beginCrossing(){
  ...
  target(){
    ...
    const [object,label,text]=targets[id];return {position:object.position,label,text};
  }
};
